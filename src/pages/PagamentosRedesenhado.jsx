  ...
  return (
    <label className={`grid cursor-pointer grid-cols-[auto_1fr_auto] gap-3 border-b border-black/5 px-4 py-3 last:border-0 ${marcada ? "bg-[#EDF4F1]" : "hover:bg-[#F8FAF9]"} ${bloqueada ? "cursor-not-allowed opacity-60" : ""}`}>
      ...
      <span className="text-right">
        ...
        <strong className="tabular-nums text-[#17352F]">{formatBRL(conta.saldoHoje)}</strong>
      </span>
    </label>
  );
}
...
export default function PagamentosRedesenhado() {
  ...
  async function carregarBase() {
    try {
      ...
      const saldos = await buscarSaldoRealPorConta({ contaIds: (contasBrutas ?? []).map((conta) => conta.id) });
      const nomeSecretaria = secretarias.find((item) => String(item.id) === String(secretariaId))?.nome || "--";
      setContas(montarSaldosDasContas((contasBrutas ?? []).map((conta) => ({
        id: conta.id,
  ...
  async function confirmarConcentracao() {
    ...
    const excedente = itens.find((item) => item.amount > numero(contas.find((conta) => String(conta.id) === String(item.sourceAccountId))?.saldoHoje));
  ...
  async function gerarPdf() {
    ...
    autoTable(doc, {
      ...
      body: contasSelecionadasComSaldo.map((conta) => [conta.nome_conta, conta.banco, conta.numero_conta || "--", formatBRL(conta.saldoHoje)]),
  ...
  const totalDisponivel = React.useMemo(() => somar(contasSelecionadasComSaldo.map((conta) => conta.saldoHoje)), [contasSelecionadasComSaldo]);
