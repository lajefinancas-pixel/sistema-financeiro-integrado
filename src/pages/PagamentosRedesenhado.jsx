...
export default function PagamentosRedesenhado() {
  ...
  const linhasProposta = React.useMemo(() => linhasDaProposta(pagamentos), [pagamentos]);
  const chavesPagamentos = React.useMemo(
    () => new Map(linhasProposta.map((linha) => [linha.pagamento, linha.chave])),
    [linhasProposta],
  );
        ...
        {carregando ? <p className="py-12 text-center text-[13px] text-[var(--color-brand-navy)]/55">Carregando...</p> : <>
          ...
          {!programacao ? <div className="rounded-xl border border-dashed border-[var(--color-brand-navy)]/20 bg-white/60 px-4 py-12 text-center"><h2 className="font-serif text-lg text-[var(--color-brand-navy)]">Comece uma programação diária</h2><p className="mt-1 text-[12px] text-[var(--color-brand-navy)]/55">Planejamento apenas: nenhuma conta é debitada ou bloqueada.</p></div> : <>
            ...
            <div className="grid gap-3 xl:grid-cols-[1.05fr_.95fr]">
              ...
              <section className="overflow-hidden rounded-xl border border-[var(--color-brand-navy)]/10 bg-white shadow-sm">
                ...
                <div className={fornecedoresConfirmados ? "" : "hidden print:block"}>
                  {emEtapaDeExecucao ? <LinhasExecucaoProgramacao pagamentos={pagamentosOrdenados} nomePagamento={nomePagamento} podeMarcar={podeEditar && permissoesFase2?.executar_programacao !== false} salvando={salvando} onMarcar={marcarSituacao}/> : <>{pagamentos.length === 0 ? <p className="px-3 py-5 text-center text-[13px] text-[var(--color-brand-navy)]/45">Nenhum fornecedor escolhido.</p> : linhasProposta.map(({ pagamento, chave }, indice) => <div key={chave} data-item-programacao={chave} className="grid gap-1 border-b border-black/5 px-3 py-1 text-[13px] leading-tight last:border-0 sm:grid-cols-[1fr_9rem_auto] sm:items-center sm:gap-2"><div className="min-w-0">{nomeDoItem(pagamento, indice)}{pagamento.cadastrar_fornecedor_posteriormente && <small className="text-[10px] text-[#A5542F]">Cadastrar posteriormente</small>}{etiquetaDeOrigem(pagamento)}</div><CampoMoeda valor={pagamento.valor_a_pagar} onValorChange={(valor) => editarValor(pagamento, valor)} aria-label={`Valor a pagar para ${nomePagamento(pagamento)}`} className="w-full rounded-lg border border-black/10 px-2 py-1 text-right text-[13px] font-bold normal-case tracking-normal text-[var(--color-brand-navy)] print:hidden"/><strong className="hidden text-right tabular-nums print:block">{formatBRL(pagamento.valor_a_pagar)}</strong><button onClick={() => setPagamentos((itens) => itens.filter((item) => item !== pagamento))} className="rounded p-1 text-red-600 hover:bg-red-50 print:hidden" aria-label={`Retirar ${nomePagamento(pagamento)} da programação`}><Trash2 size={14}/></button></div>)}<div className="bg-[var(--color-brand-navy)] px-3 py-1.5 text-[11px] font-bold tracking-[0.04em] text-white">{pagamentos.length} {pagamentos.length === 1 ? "FORNECEDOR ESCOLHIDO" : "FORNECEDORES ESCOLHIDOS"} — TOTAL PROGRAMADO: {formatBRL(totalProgramado)}</div></>}
                </div>
            ...
            {!fornecedoresConfirmados && <section className="mt-3 overflow-hidden rounded-xl border border-[var(--color-brand-navy)]/10 bg-white shadow-sm print:hidden">
              ...
              {pagamentos.length === 0 ? <p className="px-3 py-6 text-center text-[13px] text-[var(--color-brand-navy)]/45">Selecione fornecedores ou adicione um avulso.</p> : <div>{linhasProposta.map(({ pagamento, chave }, indice) => <div key={chave} data-item-programacao={chave} className="grid gap-1 border-b border-black/5 px-3 py-1 text-[13px] leading-tight last:border-0 sm:grid-cols-[1fr_9rem_auto] sm:items-center sm:gap-2"><div className="min-w-0">{nomeDoItem(pagamento, indice)}{pagamento.cadastrar_fornecedor_posteriormente && <small className="text-[10px] text-[#A5542F]">Cadastrar posteriormente</small>}{etiquetaDeOrigem(pagamento)}</div><CampoMoeda valor={pagamento.valor_a_pagar} onValorChange={(valor) => editarValor(pagamento, valor)} aria-label={`Valor a programar para ${nomePagamento(pagamento)}`} className="w-full rounded-lg border border-black/10 px-2 py-1 text-right text-[13px] font-bold normal-case tracking-normal text-[var(--color-brand-navy)]"/><button onClick={() => setPagamentos((itens) => itens.filter((item) => item !== pagamento))} className="rounded p-1 text-red-600 hover:bg-red-50" aria-label={`Retirar ${nomePagamento(pagamento)} da programação`}><Trash2 size={14}/></button></div>)}</div>}
            </section>}
