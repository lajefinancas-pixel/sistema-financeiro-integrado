-- Reaplicação idempotente da estrutura da Fase 1 dos Pagamentos Diários.
--
-- Por que existe: no projeto de produção o registro de migrations marca
-- 20260827000000_consolidar_fluxo_pagamentos_diarios.sql como aplicado, mas os
-- objetos que aquela migration cria não estão no banco. Conferido pelo próprio
-- PostgREST do projeto da aplicação:
--
--   programacoes_pagamento.status                    -> 42703 (não existe)
--   programacoes_pagamento.saldo_considerado         -> 42703
--   programacoes_pagamento.total_programado          -> 42703
--   programacoes_pagamento.restante                  -> 42703
--   programacoes_pagamento.updated_at                -> 42703
--   programacao_contas.saldo_considerado             -> 42703
--   programacao_contas.ativa                         -> 42703
--   pagamentos.cadastrar_fornecedor_posteriormente   -> 42703
--   salvar_planejamento_programacao / marcar_programacao_em_analise -> PGRST202
--
-- (programacoes_pagamento.ultima_impressao_em já existe: o banco recebeu uma
-- revisão parcial daquele arquivo.) Como o nome já consta como aplicado, rodar
-- de novo o arquivo original é ignorado pela ferramenta de migration -- por
-- isso a correção vem para frente, em arquivo novo, sem editar nada aplicado.
--
-- Tudo aqui é idempotente e não movimenta dinheiro: só acrescenta colunas de
-- planejamento, índices e as duas funções que a tela chama. Não toca em
-- saldos_historico, transferências, baixas, certidões, tarefas nem
-- configurações.

begin;

-- Precondições do que esta migration escreve.
do $$
declare
  item record;
  tipo_real text;
begin
  ...
  if to_regclass('public.auditoria_eventos') is null then
    raise exception 'Estrutura incompatível: public.auditoria_eventos não existe.';
  end if;
end $$;
